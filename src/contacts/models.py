...
class Friend(models.Model):
    ...
    user_social_auth = models.ForeignKey(UserSocialAuth, related_name='friends', default=0, on_delete=models.PROTECT)
    ...
    def fetch(social_user, request):
        ...
        if social_user.provider == 'facebook':
            ...
            Friend.fetch_microsoft(request, social_user)
        elif social_user.provider == 'google-oauth2':
            Friend.fetch_google(social_user)
        elif social_user.provider == 'yahoo-oauth2':
            Friend.fetch_yahoo(social_user)

    @staticmethod
    def fetch_yahoo(social_user):
        ...
        for contact in page.json()['contacts']['contact']:
            ...
            if social_id and name and not Friend.objects.filter(social_id=social_id).exists():
                Friend(social_id=social_id, name=name, user_social_auth=social_user).save()

    @staticmethod
    def fetch_google(social_user):
        ...
        people = service.people().connections().list(  # pylint: disable=no-member
            resourceName='people/me', personFields='names,emailAddresses').execute()
        for item in people.get('connections'):
            social_id = item['resourceName'].split('/')[1]
            name = item.get('names', [{}])[0].get('displayName')
            if social_id and name and not Friend.objects.filter(social_id=social_id).exists():
                Friend(social_id=social_id, name=name, user_social_auth=social_user).save()

    @staticmethod
    def fetch_microsoft(request, social_user):
